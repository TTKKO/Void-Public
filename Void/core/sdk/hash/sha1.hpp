...
namespace sdk::hash
{
	class sha1
	{
		void add_byte_dont_count_bits(const std::uint8_t x)
		{
			buf[i++] = x;
		...
		static std::uint32_t rol32(const std::uint32_t x, const std::uint32_t n)
		{
			return (x << n) | (x >> (32 - n));
		}

		static std::uint32_t make_word(const std::uint8_t* p)
		{
			return static_cast<std::uint32_t>(p[0]) << 3 * 8 | static_cast<std::uint32_t>(p[1]) << 2 * 8 | static_cast<std::uint32_t>(p[2]) << 1 * 8 | static_cast<std::uint32_t>(p[3]) << 0 * 8;
		}

		void process_block(const std::uint8_t* ptr)
		{
			constexpr std::uint32_t c0 = 0x5a827999;
			constexpr std::uint32_t c1 = 0x6ed9eba1;
			constexpr std::uint32_t c2 = 0x8f1bbcdc;
			constexpr std::uint32_t c3 = 0xca62c1d6;
...
#undef SHA1_ROUND_4
	...
	public:
		...
		explicit sha1(const char* text = nullptr) : i(0), n_bits(0)
		{
			state[0] = 0x67452301;
			state[1] = 0xEFCDAB89;
			state[2] = 0x98BADCFE;
		...
		sha1& add(char c)
		{
			return add(*reinterpret_cast<std::uint8_t*>(&c));
		}

		sha1& add(const void* data, std::uint32_t n)
		{
			if (!data)
				return *this;
		...
		{
			if (!text) return *this;
			return add(text, static_cast<std::uint32_t>(strlen(text)));
		}
		...
		sha1& finalize()
		{
			...
			for (int j = 7; j >= 0; j--)
				add_byte_dont_count_bits(static_cast<std::uint8_t>(n_bits >> j * 8));
		...
		) const
		{
			// print hex
			std::int32_t k = 0;
			for (const std::uint32_t i : state)
			{
		...
		const sha1& print_base64(char* base64, const bool zero_terminate = true) const
		{
			static const auto* table = reinterpret_cast<const std::uint8_t*>("ABCDEFGHIJKLMNOPQRSTUVWXYZ"
				"abcdefghijklmnopqrstuvwxyz"
				"0123456789"
				"+/");

			const std::uint32_t triples[7] = {
			...
			{
				const std::uint32_t x = triples[i];
